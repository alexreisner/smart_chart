...
# Strengthen distinction between attributes and parameters: rename parameter-fetching methods to mimick Google's names. We don't want any confusion between the two, and we want full attr_accessors for attributes since we may not be able to assign their entire values at once (eg, assign one line style, then add more as we interate through datasets).

# Parameter methods should not output their name (eg, "chd=", "chs="); they should be called by a dispatcher that prefixes their output (if non-blank). Subclasses of Base should implement whatever parameter methods they need.

# Use modules as mix-ins to package common parameter method implementations.
