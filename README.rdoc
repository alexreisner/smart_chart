...
== Examples
  ...
  SmartChart::LineGraph.new(
    ...
    # x-gridlines (style specified by LineStyle hash or symbol)
    :x_grid => {
      :step   => 10,
      :offset => 2,
      :style  => :dashed
...
1. Strengthen distinction between attributes and parameters: rename parameter-fetching methods to mimick Google's names. We don't want any confusion between the two, and we want full attr_accessors for attributes since we may not be able to assign their entire values at once (eg, assign one line style, then add more as we interate through datasets).

2. Parameter methods should not output their name (eg, "chd=", "chs="); they should be called by a dispatcher that prefixes their output (if non-blank). Subclasses of Base should implement whatever parameter methods they need.

3. Use modules as mix-ins to package common parameter method implementations.
...
* Simulate sparkline bar graphs (SparkBar) by hiding 1px from left and bottom of image (display as bg image of a div when to_html is called).
